...
(async () => {
  try {
    ...
    const users = [
      ...
      { username: 'manager', password: 'abcd', role: 'manager', squad: 'B' },
      { username: 'user',    password: '0000', role: 'user', squad: 'B' },
      { username: 'sara',   password: '2006', role: 'user', squad: 'A' },
      { username: 'ahmed',    password: '3007', role: 'manager', squad: 'A' },
      { username: 'paul',    password: '4008', role: 'user', squad: 'A' },
      { username: 'lisa',    password: '5009', role: 'user', squad: 'A' }
    ];
    ...
    for (const user of users) {
      ...
      if (rows.length > 0) {
        //console.log(  L'utilisateur '${user.username}' existe déjà — insertion ignorée.`);
      } else {
        // 2.3 Insertion dans la table users
        await pool.query(
          'INSERT INTO users (username, password, role, squad) VALUES (?, ?, ?, ?)',
          [user.username, hashedPassword, user.role, user.squad || null]
        );
        console.log(` Utilisateur '${user.username}' ajouté avec succès.`);
      }
    }

    //  Fermer la connexion si la méthode existe (pool.end est optionnelle selon ton implémentation)
