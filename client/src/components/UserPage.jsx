...
export default function UserPage({ user, onLogout }) {
  ...
  return (
    <div className="card">
      ...
      <div style={{maxWidth:480,margin:'32px auto 24px auto',background:'#fff',borderRadius:16,boxShadow:'0 2px 16px #0001',padding:24}}>
        <div style={{fontWeight:600,fontSize:18,marginBottom:12}}>Ajouter une vente</div>
        <form onSubmit={submitVente} style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
          <input className="input-modern" placeholder="ID produit" value={form.id_produit} onChange={e=>setForm({...form,id_produit:e.target.value})} required style={{padding:12,borderRadius:8,border:'1px solid #e2e8f0',fontSize:16}} />
          <input className="input-modern" placeholder="Quantité" value={form.quantite} onChange={e=>setForm({...form,quantite:e.target.value})} required style={{padding:12,borderRadius:8,border:'1px solid #e2e8f0',fontSize:16}} />
          <input className="input-modern" placeholder="Adresse de livraison" value={form.adresse} onChange={e=>setForm({...form,adresse:e.target.value})} required style={{gridColumn:'1/3',padding:12,borderRadius:8,border:'1px solid #e2e8f0',fontSize:16}} />
          <div style={{gridColumn:'1 / -1',display:'flex',gap:12,justifyContent:'flex-end',marginTop:8}}>
            <button type="submit" style={{background:'#22c55e',color:'#fff',border:'none',borderRadius:8,padding:'10px 18px',fontWeight:600,cursor:'pointer'}}>Ajouter</button>
          </div>
        </form>
        {message && <div style={{color:message.includes('ajoutée')?'#22c55e':'crimson',marginTop:10,border:'1px solid #eee',background:'#f8fafc',padding:8,borderRadius:8}}>{message}</div>}
      </div>
